...
def main():
    ...
    HML_L_ARM_JOINTS = [humanml_utils.HML_JOINT_NAMES.index(name) for name in [
        "left_shoulder", "left_elbow", "left_wrist"
    ]]
    HML_L_ARM_JOINTS_BINARY = np.array([i not in HML_L_ARM_JOINTS for i in range(humanml_utils.NUM_HML_JOINTS)])

    NOT_L_ARM_MASK = np.concatenate(([True]*(1+2+1),
    ...
    if "only allow arm to move": # if True lol
        model_kwargs['y']['inpainting_mask'] = torch.tensor(NOT_L_ARM_MASK, dtype=torch.bool,
    ...
    np.save(npy_path,
            {'motion': all_motions, 'text': all_text, 'lengths': all_lengths, 'samples': (torch.cat(samples)).cpu().numpy(),
    ...
    for sample_i in range(args.num_samples):
        ...
        input_clip = plot_3d_motion(animation_save_path, skeleton, motion, title=caption,
        ...
        input_clip.write_videofile(animation_save_path, fps=fps, codec="libx264",
                                   audio=False, logger=None)
        input("after plotting")
        for rep_i in range(args.num_repetitions):
            ...
            animations[sample_i, rep_i] = animations[sample_i, rep_i].set_duration(float(length) / fps)
            animations[sample_i, rep_i].write_videofile(animation_save_path, fps=fps,
