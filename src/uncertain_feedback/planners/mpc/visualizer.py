...
class ArmVisualizer:
    ...
    def animate(
        ...
        for col, (title, elev, azim) in enumerate(_3D_VIEWS):
            ...
            ax.scatter(*tpose[_BODY_JOINTS].T, color=_BODY_COLOR, s=14,
                       alpha=0.45, depthshade=False)

            # Static target arm
            _draw_bones_3d(ax, target_full, LEFT_ARM_BONE_PAIRS_22,
                           _TARGET_COLOR, alpha=0.4, lw=1.8, linestyle="--",
                           label="target" if col == 0 else None)
            ax.scatter(*target_full[LEFT_ARM_JOINT_INDICES_22].T,
                       color=_TARGET_COLOR, s=30, alpha=0.4, depthshade=False)

            if col == 0:
                ax.legend(loc="upper left", fontsize=7)

            # Mutable current arm
            scat = ax.scatter([], [], [], color=_CURRENT_COLOR, s=40,
                              depthshade=False, zorder=5,
                              label="current" if col == 0 else None)
            lines = [ax.plot([], [], [], color=_CURRENT_COLOR, lw=1.8)[0]
                     for _ in LEFT_ARM_BONE_PAIRS_22]
            trace, = ax.plot([], [], [], color=_TRACE_COLOR, lw=1,
                             alpha=0.6, linestyle=":")

            artists_3d.append({"scat": scat, "lines": lines, "trace": trace,
                                "ax": ax, "is_main": col == 0})
        ...
        for col, (title, hi, vi, hl, vl) in enumerate(_ORTHO_VIEWS):
            ...
            ax2.set_xlabel(hl, fontsize=8); ax2.set_ylabel(vl, fontsize=8)
            ax2.set_xlim(*lims[hi]); ax2.set_ylim(*lims[vi])
            ax2.tick_params(labelsize=7)

            # Static body
            _draw_bones_2d(ax2, tpose, _BODY_BONES, hi, vi, _BODY_COLOR,
                           alpha=0.45, lw=1.2)
            ax2.scatter(tpose[_BODY_JOINTS, hi], tpose[_BODY_JOINTS, vi],
                        color=_BODY_COLOR, s=14, alpha=0.45, zorder=3)

            # Static target arm
            _draw_bones_2d(ax2, target_full, LEFT_ARM_BONE_PAIRS_22, hi, vi,
                           _TARGET_COLOR, alpha=0.4, lw=1.8, linestyle="--")
            ax2.scatter(target_full[LEFT_ARM_JOINT_INDICES_22, hi],
                        target_full[LEFT_ARM_JOINT_INDICES_22, vi],
                        color=_TARGET_COLOR, s=28, alpha=0.4, zorder=4)

            # Mutable current arm
            scat  = ax2.scatter([], [], color=_CURRENT_COLOR, s=35, zorder=5)
            lines = [ax2.plot([], [], color=_CURRENT_COLOR, lw=1.8)[0]
                     for _ in LEFT_ARM_BONE_PAIRS_22]
            trace, = ax2.plot([], [], color=_TRACE_COLOR, lw=1,
                              alpha=0.6, linestyle=":")

            artists_2d.append({"scat": scat, "lines": lines, "trace": trace,
                                "hi": hi, "vi": vi})
    ...
    def plot_pose(
        ...
        ax.scatter(*tpose[_BODY_JOINTS].T, color=_BODY_COLOR, s=20,
                   alpha=0.5, depthshade=False)
        _draw_bones_3d(ax, pos, LEFT_ARM_BONE_PAIRS_22, _CURRENT_COLOR,
                       alpha=1.0, lw=2, label="current")
        ax.scatter(*pos[LEFT_ARM_JOINT_INDICES_22].T, color=_CURRENT_COLOR,
                   s=45, depthshade=False)

        if target_q is not None:
            tgt = self.fk.full_body_positions(target_q, spine3_pos, spine3_aa)
            _draw_bones_3d(ax, tgt, LEFT_ARM_BONE_PAIRS_22, _TARGET_COLOR,
                           alpha=0.4, lw=2, linestyle="--", label="target")
            ax.scatter(*tgt[LEFT_ARM_JOINT_INDICES_22].T, color=_TARGET_COLOR,
                       s=35, alpha=0.4, depthshade=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save",     type=str, default=None, help="Save path (e.g. arm.gif)")
